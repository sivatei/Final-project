...
const Khmer = () => {
  return (
    <div>
        <div className='w-full h-410 bg-orange-50'>
            <div className='flex gap-100'>
                <Link to="/"><button className='w-10 h-10 bg-orange-100 text-2xl rounded-full shadow-lg my-8 mx-10 hover:scale-105 transition'><IoMdArrowRoundBack className='mx-2'/></button></Link>
            ...
            <div className='flex flex-wrap gap-8 justify-center'>
                {khmerFoodData.map((KHF, index) => (
                    <div key={KHF.id} className='w-70 h-112 bg-white rounded-2xl shadow-lg mt-5'>
                        ...
                        <div className='text-center mt-4'>
                            <Link to={index === 0 ? `/amok` : `/plan`}><button className='w-[90%] h-10 border-2 border-orange-500 rounded-xl text-orange-500 text-sm hover:bg-linear-to-r from-orange-500 to-orange-600 hover:text-white transition duration-250'>VIEW RECIPE</button></Link>    
                        </div>
                    </div>
                ))}
            </div>
